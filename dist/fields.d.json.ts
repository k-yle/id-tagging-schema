 ...
 */
export type Field = (
  | FieldTypeWithoutKeyNorKeys
  | FieldTypeWithKeyOptionalKeys
  | FieldTypeWithKeyAndKeys
  | FieldTypeWithKeyOrKeys
) & {
  /**
  ...
  prerequisiteTag?:
    | RequiresKeyAnyValue
    | RequiresKeyEqualsValue
    | RequiresKeyEqualsValues
    | RequiresKeyNotValue
    | RequiresNotKey;
  /**
...
export type FieldTypeWithoutKeyNorKeys = {
  [k: string]: unknown;
} & {
  type?: "restrictions";
};
export type FieldTypeWithKeyOrKeys = {
  [k: string]: unknown;
};
export type Geometry = "point" | "vertex" | "line" | "area" | "relation";

export interface FieldTypeWithKeyOptionalKeys {
  type?: "email" | "url" | "tel" | "text" | "number";
}
export interface FieldTypeWithKeyAndKeys {
  type?: "address" | "wikipedia" | "wikidata" | "directionalCombo";
}
...
export interface RequiresKeyEqualsValues {
  ...
  key: string;
  /**
   * The values that the tag must have. (alternative to 'valueNot')
   */
  values: string[];
}
